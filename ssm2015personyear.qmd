...
df1 |> 
  select(id, year, age, age2014, birthyear, birthmonth) |> 
  filter(birthmonth == 1) 
...
df2_1 <-
  df1 |> 
  # expandしたデータに職歴の情報をくっつける
  left_join(occ, by = join_by(id, age)) |> 
  # idごとに次のnumまでのNAを埋める
  group_by(id) |> 
  fill(
    jobnum_, matches('job|nfirm|industry|firmsize|status|occupation|title|jobst|joben|reason|reason_other'),
    .direction = 'down'
  ) |> 
  ungroup() |> 
  # 就業状態を表す変数（work）を作成
...
df2_2 |> 
  select(id, age, school_enrolled, schoolnum_, schoolst, schoolen) |> 
  filter(school_enrolled == 1) 
...
df2_3 |>
  select(id, age, age_firstmarriage, age_recentmarriage, age_divorce, age_separate, marstat)
...
df2_4 |>
  select(id, year, birthyear_child1, birthyear_child2, birthyear_child3, age_child1, age_child2, age_child3, number_child)
...
## 従業上の地位による初婚への意向の違いに関するイベントヒストリー分析
